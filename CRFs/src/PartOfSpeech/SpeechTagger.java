...
public class SpeechTagger {
    ...
    String inName="partSpeech";
    String outDir="partSpeech";
    String baseDir="my_samples/PartSpeechSamples";
    int nlabels=37;
    ...
    public static void main(String argv[]) throws Exception {
        ...
        SpeechTagger tagger = new SpeechTagger();
    ...
    public void train() throws Exception {
	     ...
	     */
             PartSpeechIter trainData=encapsulateData(baseDir+"/data/"+inName+"/"+inName+".train.tagged");
	    /*
    ...
    void  allocModel() throws Exception {
	     ...
	     */
	    featureGen = new FeatureGenImpl("naive",nlabels);
	    crfModel = new CRF(featureGen.numStates(),featureGen,options);
    }
    public PartSpeechIter encapsulateData(String fname) throws FileNotFoundException{
        String line;
        String[] pair;
        ArrayList<Object> x = new ArrayList();
        ArrayList<Integer> y = new ArrayList<Integer>();
        ArrayList<PartSpeechSequence> sequences = new ArrayList();
        Scanner in = new Scanner(new File(fname));
        while (in.hasNextLine()){
            line = in.nextLine();
            if (line.equals("")){
                if (x.size()>0){
                    Integer[] z = new Integer[y.size()];
                    sequences.add(new PartSpeechSequence(x.toArray(), y.toArray(z)));
                    x.clear();
                    y.clear();
                }
            }else{
                pair = line.split("["+tagDelimit+"]");
                //System.out.println(pair[0]+" "+pair[1]);
                x.add(pair[0].trim());
                y.add(Integer.parseInt(pair[1].trim())-1);
            }
            /*if (sequences.size()>0)
                System.out.println(sequences.get(sequences.size()-1).toString());
             */
        }
        if (x.size()>0){
            Integer[] z = new Integer[y.size()];
        ...
        PartSpeechSequence[] a = new PartSpeechSequence[sequences.size()];
        return new PartSpeechIter(sequences.toArray(a));
    }
